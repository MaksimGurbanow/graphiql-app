...
const TestComponent = () => {
    ...
    return (
        <div>
            ...
            <button onClick={() => setRest({ url: 'new-rest-url', method: 'POST', headers: [], body: '', variables: [], params: [] })}>
                Set REST URL
            </button>
            <button onClick={() => setGraphql({ url: 'new-graphql-url', query: 'query { ... }', response: '', headers: [], variables: {} })}>
                Set GraphQL URL
            </button>
        </div>
    );
...
describe('RequestProvider', () => {
    ...
    it('provides context values and updates correctly', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByTestId('graphQL-url')).toHaveTextContent('new-graphql-url');
        });
    });
