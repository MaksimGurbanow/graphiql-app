...
} | { kind: string; name: string; fields: { name: string; type: { kind: string; name: string } }[] } | {
    kind: string;
    name: string;
    description: string
})[] = [
...
describe('ExplorerList Component', () => {
    ...
    it('renders fields of Query when Query is the last segment', () => {
        renderWithProps([{ name: 'Root', type: 'Root' }, { name: 'query', type: 'Query' }]);
    ...
    it('renders fields of Mutation when Mutation is the last segment', () => {
        renderWithProps([{ name: 'Root', type: 'Root' }, { name: 'mutation', type: 'Mutation' }]);
        expect(screen.getByText(/mutate1: Boolean/i)).toBeInTheDocument();
    });

    it('handles button click to navigate to the next segment', () => {
        const { setPathSegments } = renderWithProps([{ name: 'Root', type: 'Root' }]);
    ...
    it('navigates correctly when clicking on a field of Mutation type', () => {
        ...
        const { setPathSegments } = renderWithProps([{ name: 'Root', type: 'Root' }, { name: 'mutation', type: 'Mutation' }]);
        ...
        const updateFunction = setPathSegments.mock.calls[0][0];
        const newPathSegments = updateFunction([{ name: 'Root', type: 'Root' }, { name: 'mutation', type: 'Mutation' }]);
        expect(newPathSegments).toEqual([
            { name: 'Root', type: 'Root' },
    ...
    it('handles navigation through nested object types correctly', () => {
        const { setPathSegments } = renderWithProps([{ name: 'Root', type: 'Root' }, { name: 'query', type: 'Query' }]);
        ...
        const updateFunction = setPathSegments.mock.calls[0][0];
        const newPathSegments = updateFunction([{ name: 'Root', type: 'Root' }, { name: 'query', type: 'Query' }]);
        expect(newPathSegments).toEqual([
            { name: 'Root', type: 'Root' },
