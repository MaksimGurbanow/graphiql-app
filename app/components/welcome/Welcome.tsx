...
const Welcome = () => {
  ...
  return (
      <div className={styles.aboutUsContainer}>
        <h1 className={`${styles.welcomeTitle} ${isAnimating ? styles.animate : ""}`}>
          {t("welcome.reactGroup")}
        </h1>
        <div className={styles.audioBlock}>
          <div>
            <h2 className={styles.audioHeading}>{t("welcome.prepareTogether")}</h2>
        ...
        {isMobileView ? (
            <Swiper>
              {teamMembers.map((member) => (
                  <div
                      key={member.name}
                      className={`${styles.teamMember} ${isAnimating ? styles.animate : ""}`}
                  >
                    <img src={member.photo} alt={`Фото ${t(`welcome.${member.name}`)}`} className={styles.teamMemberPhoto} />
